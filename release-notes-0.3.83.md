...
- **应用内重启覆盖 AOD 进程**：模块作用域含 SystemUI、`android`、`com.miui.aod` 三个包，但"重启"动作原先只杀 SystemUI，更新模块后 MiuiAOD 仍跑旧代码导致亮度 hook 迟迟不生效。现在先杀 AOD 再杀 SystemUI，一次生效。
...
- **诊断采集窗口按时间过滤**：logcat 从 `-t 4000`（行数截断）改为 `-T <捕获起始时间>`，只收集捕获开始后的日志；引导文案明确要求先复现故障再完成捕获。
- **文档全面双语化**：11 份文档（FAQ、数据政策、架构、诊断规范、行为规范、风格指南、发布规范等）全部改为中英双语；"报告问题"相关文档与 App 当前实际对齐（7 个报告分类、三包作用域、四歌词源）。
- 上游 `8422d78`（v0.3.97）评估收尾：辅助行来源、假名拒绝、fillEnd 钳制补入行为规范文档；测试增至 586 个全部通过。
