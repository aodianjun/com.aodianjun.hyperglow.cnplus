...
- Lyrics on the HyperOS lock screen and AOD from multiple sources / 通过多种来源在 HyperOS 锁屏与 AOD 上显示歌词：
  - **Spicy EX** (Spotify, international / 国际版，Spotify).
  - **Lyricon** (popular Chinese music apps — QQ音乐、网易云音乐、酷狗音乐等).
- Line-, word- and syllable-synchronized karaoke / 支持逐行、逐词、逐音节同步的卡拉OK.
- Transliteration and translation with Spicy EX Full / 搭配 Spicy EX Full 支持音译与翻译.

- AOD clock placement and burn-in movement / AOD 时钟位置与防烧屏位移.
- Keep AOD active while lyrics are visible / 歌词显示时保持 AOD 常亮.
- Keep the lock screen awake while music is playing / 播放音乐时保持锁屏常亮.
- Raise to show AOD instead of the full lock screen / 拿起手机显示 AOD 而非完整锁屏.

## Requirements / 环境要求

- Rooted HyperOS 3 / 已 root 的 HyperOS 3.
- [LSPosed](https://github.com/LSPosed/LSPosed).
- At least one lyrics source / 至少一个歌词源：
  - Spotify with Spicy EX Lite or Full (**Publish lyrics to HyperGlow** enabled) / Spotify + Spicy EX Lite 或 Full（需开启 **将歌词发布到 HyperGlow**）。
  - A Chinese music app with [Lyricon](https://github.com/tomakino/lyricon) active in SystemUI / 国内音乐软件 + [Lyricon](https://github.com/tomakino/lyricon)（在 SystemUI 作用域启用）。
...
1. Enable HyperGlow in LSPosed / 在 LSPosed 中启用 HyperGlow.
2. Enable your lyrics source / 启用你的歌词源:
   - **Spotify**: enable Spicy EX for Spotify in LSPosed, then enable **Publish lyrics to HyperGlow** in Spicy EX / 在 LSPosed 中为 Spotify 启用 Spicy EX，然后在 Spicy EX 中开启 **将歌词发布到 HyperGlow**。
   - **Chinese music apps**: enable [Lyricon](https://github.com/tomakino/lyricon) for SystemUI in LSPosed / 国内音乐软件：在 LSPosed 中为 SystemUI 启用 [Lyricon](https://github.com/tomakino/lyricon)。
3. Set HyperGlow battery usage to **No restrictions** / 将 HyperGlow 的电池使用设置为 **无限制**。
...
- Changes are limited to what exists here. A pull request that adds files outside this tree cannot
  be integrated as written / 改动仅限于本仓库已有的内容。新增超出本目录文件的 PR 无法按原样合并。
- `README.md`, `FAQ.md`, and `.gitignore` are generated. Edits to them are lost; raise the change in
  an issue instead / `README.md`、`FAQ.md` 和 `.gitignore` 为自动生成，对它们的修改会被丢弃；请改为在 issue 中提出。
- Every accepted change is verified on the maintainer's device before release. Unit tests passing is
  ...
  hardware verification that cannot run in CI / 每个被接受的改动在发布前都会在维护者的设备上验证。单元测试通过是必要条件而非充分条件——任何涉及 SystemUI 挂钩、AOD 电源或几何布局的改动都需要无法在 CI 中运行的硬件验证。
- Large or architectural changes are worth discussing in an issue first, so the design can be
  checked against the specs before you build it / 大型或架构性改动值得先在 issue 中讨论，以便在动手前对照规范检查设计方案。
