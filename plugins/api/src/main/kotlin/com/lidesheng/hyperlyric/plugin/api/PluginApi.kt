...
/**
 * The HyperLyric plugin API, version 1.
 *
 * This is a FQCN-compatible copy of HyperLyric's plugin contract
 * (https://github.com/limczhh/HyperLyric, GPL-3.0), so that HyperLyric plugin
 * ZIP packages (manifest.json + classes.dex) load in the HyperGlow runtime
 * without recompilation. The package name and every public declaration below
 * must stay byte-identical to the upstream API; do not rename or renumber.
 *
...
public interface LyricProcessorExtension : HyperLyricExtension {
     ...
     * declared in the returned result; the host never infers changed fields by comparing DTOs.
     */
    public fun processResult(song: PluginSong): PluginSongResult? = null

    /**
     * Process with a host-owned, read-only media snapshot. Existing processors that only
...
/** Read-only media fields assembled by the host for network lookup and prompt construction. */
...
/** Per-invocation data supplied by the host without exposing Android, MediaSession or Xposed types. */
public data class PluginProcessingContext(
    val mediaInfo: PluginMediaInfo? = null,
)

/** Read-only from the plugin's point of view; values are changed by the host App. */
 ...
 *
 * [changedFields] is authoritative. A nullable value in [song] is copied into the host only when
...
/** Semantic setting types used by the host renderer, independent of UI toolkit class names. */
...
public data class PluginSettingSpec(
    ...
    /** Whether this setting value may be included in a full host backup. */
