...
**Bottom line**: no mandatory port among the nine actionable items. Highest-value, lowest-risk is **#4 ConfigBackupCodec/SettingsSession**; independent low-risk ports worth taking now are **#7 (customization cache)** and **#9 (brightness override, CN+ already has the controller)**. **#2** warrants a verify-first check against CN+'s existing suppression; **#5 (duet)** and **#6 (interlude)** are not directly portable until CN+'s document model gains concurrent rows / a `role` field; **#1 and #3** are deferred; **#8/#11** follow #1/#2. After any port, re-run the API-contract fingerprint and full CI (554+ tests), then update this file.
...
| 6 | **`hasActualLyricTiming` interlude 修复**——乐器点(u0022点)行不再计入"实际计时"，避免纯过门文档把 AOD 场景/keepalive 拉住 | AodProjectionEngine（+11，`it.role != ROW_ROLE_INTERLUDE`） | 中（场景存活边缘场景） | **不可直接移植**：CN+ 行模型无 `role`/`ROW_ROLE_INTERLUDE`。仅当给 CN+ 文档行增加 `role` 字段后再移植 |
| 7 | **CustomizationRepository 编译缓存 + 失效**——内存缓存编译结果，仅当持久输入（当前/上一文档、legacy 配置）变化才重新解码/编译，降低 10 Hz tick 的 CPU/分配；用"始终规范化"取代版本号迁移标志 | CustomizationRepository（+129） | 中（性能） | **可选移植（低风险、自包含）**——快速路径对任何其他特性都独立成立 |
| 8 | **AOD 最大高度默认 0.42→0.9**——`SurfacePolicyResolver` AOD `maximumHeightFraction 0.5→0.9`、`canonicalizeDocument` v2 迁移（legacy AOD 0.42→新默认）、`fromLegacyConfigFile` AOD 映射到新默认 | SurfacePolicyResolver、CustomizationRepository、SceneCompiler | 中 | **已移植（部分，2026-09-16）**——`SurfacePolicyResolver` AOD 硬上限 `0.5→0.9` 已放开。**`canonicalizeDocument`/`fromLegacyConfigFile` 迁移对 CN+ 不适用**：CN+ 的 `SceneCompiler.compileProfile` 在编译时把 AOD 内部 `maxHeightFraction` 强制为 `AOD_FIXED_MAX_HEIGHT_FRACTION=0.3`（内容贴合），与名义值无关，因此名义上的 0.42 对 AOD 是死值，迁移成 0.9 无运行时效果。内部高度按设计保持内容贴合，仅放开运行时硬上限 |
| 9 | **按场景 AOD 亮度覆写**——`AodBrightnessController.setBrightnessOverride(enabled, level)` 由 `onCustomization` 驱动（新增 `aodBrightnessOverride`/`aodBrightnessLevel` 字段） | AodPowerCoordinator（+10）、AodBrightnessHook、CustomizationModels | 中 | **✅ 已移植（2026-09-16）**——`AodBrightnessController.setBrightnessOverride(enabled, level)` + `resolveAodBrightnessRequest` 两模式逻辑（按场景自动化钳制到可读亮度 vs 固定自定义档位，钳制 10–255）；`AodSurfaceController.onCustomization` 驱动；设置页新增总开关 **AOD 亮度增强**，其下再分**自定义亮度**开关 + 10–255 滑块（总开关开启、自定义关闭时即按场景自动化）；配置字段 `aodBrightnessOverride`/`aodBrightnessLevel` 贯通 `AodRenderPreferences`/`CustomizationModels`/`DiagnosticLogging`；新增 3 条策略测试与中英文案 |
| 10 | **新按场景定制字段**——`duetEnabled`、`cardColor`、`cardAlpha`（另含 #9 亮度覆写、#1/#2 旋转/抑制） | CustomizationModels（+67）、CustomizationRepository、SceneCompiler | 因项而异 | **选择性**：`cardColor`/`cardAlpha` 是独立的小型歌词卡片颜色/透明选项；`duetEnabled` 仅在 #5 存在时才有意义 |
...
**结论**：九项可行动项中无必须移植项。最高价值、最低风险是 **#4 ConfigBackupCodec/SettingsSession**；现在值得单独低风险移植的有 **#7（定制编译缓存）** 与 **#9（亮度覆写，CN+ 已有控制器）**。**#2** 值得先对照 CN+ 现有抑制栈核实战后再定；**#5（对唱）** 与 **#6（interlude）** 在 CN+ 文档模型获得并发行/`role` 字段前不可直接移植；**#1、#3 暂缓**；**#8/#11 跟随 #1/#2**。任何移植后都需重跑插件契约指纹与全量 CI（554+ 测试），并更新本文件。
...
| 上游提交 | 日期 | 内容 | 状态 |
|---|---|---|---|
| `748912e` 项 #8+#9 | 2026-09-16 | 按场景 AOD 亮度覆写（`AodBrightnessController.setBrightnessOverride` + 两模式设置 UI：总开关开启后 → 按场景自动化 vs 自定义 10–255 滑块）与 AOD 最大高度硬上限 `0.5→0.9`（仅 SurfacePolicyResolver；CN+ AOD 内部保持内容贴合） | ✅ 已移植（亮度）/部分（高度上限）2026-09-16 |
